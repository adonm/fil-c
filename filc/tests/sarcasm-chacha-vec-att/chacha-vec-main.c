/* OpenSSL ChaCha20 end-to-end vectors (item 8): executes the REAL
 * SARCASM-assembled tails in projects/openssl-3.6.4's libcrypto
 * (crypto/chacha/asm/chacha-x86_64.pl, all nine chacha_tail_sarcasm sites)
 * instead of the synthetic windows in sarcasm-chacha-tail-att.
 *
 * Methodology mirrors test_cha_cha_internal in
 * projects/openssl-3.6.4/test/chacha_internal_test.c: encrypt zeros with
 * the fixed key/iv (output == keystream) for every length 1..1024 and
 * compare against the upstream `ref` keystream below (first 1024 bytes,
 * copied verbatim), plus the overstep check (bytes past the length must
 * be untouched — the tail must not overrun). Every non-multiple-of-64
 * length exercises the tail of whichever implementation the host's
 * OPENSSL_ia32cap picks at runtime (ssse3/4x/8x/avx2/avx512/...); the
 * sweep hits every residue 16 times over, and the multi-block lengths
 * cover the block paths feeding the tails.
 *
 * In-test grep recipes (run-tests cannot grep; documented here, pinned at
 * runtime by the sweep): in the SARCASM=1 build
 *   rg -c 'Ltail_sarc_' crypto/chacha/chacha-x86_64.s  # 9 sites x 16 windows
 *   awk '/ChaCha20_ctr32:/,/\.Ldone/' <file> | grep -c movzb  # pristine only
 * and the per-tail scratch (%ecx/%edx) matches the register the pristine
 * tail at that site already clobbers (see chacha_tail_sarcasm).
 */
#include <stdio.h>
#include <string.h>

void ChaCha20_ctr32(unsigned char *out, const unsigned char *inp,
    unsigned long len, const unsigned int key[8],
    const unsigned int counter[4]);

static const unsigned int key[8] = {
    0x03020100, 0x07060504, 0x0b0a0908, 0x0f0e0d0c,
    0x13121110, 0x17161514, 0x1b1a1918, 0x1f1e1d1c
};

static const unsigned int ivp[4] = {
    0x00000000, 0x00000000, 0x03020100, 0x07060504
};

static const unsigned char ref[] = {
    
};

int main() {
    static unsigned char buf[1024];
    for (unsigned i = 1; i <= sizeof ref; i++) {
        memset(buf, 0, i);
        memcpy(buf + i, ref + i, sizeof ref - i);
        ChaCha20_ctr32(buf, buf, i, key, ivp);
        for (unsigned j = 0; j < sizeof ref; j++) {
            if (buf[j] != ref[j]) {
                printf("chacha vec BAD len=%u at=%u got=%02x want=%02x\n",
                    i, j, buf[j], ref[j]);
                return 1;
            }
        }
    }
    printf("chacha vectors ok\n");
    return 0;
}
